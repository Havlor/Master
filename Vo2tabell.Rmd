...
group_by(time,group) %>% # grupperer etter tid og FLS/kontroll 
  pivot_longer(names_to = "variable", 
               values_to = "value", 
               cols = weight:fmax_right) %>%
  pivot_wider(names_from = "time",
              values_from = "value") %>% # omorgansiserer tallene for å få verdier nedover
filter(!is.na(post)) %>%
  filter(!is.na(pre)) %>%#fjerner NA-verider
...
summarise(m = mean(diff),
  ...
  mutate(ms = as.character(m), paste0(signif(m, 3),  " (", signif(s, 3), ")")) %>%
select(-"m", -"s", -"ms") %>%
  pivot_wider(names_from = group,
              values_from = paste0(signif(m, 3), " (", signif(s, 3), ")")) %>%
    print()
  ...
  flextable()  %>%
   set_header_labels(variable = "",
                     ...
                     kon = "Kontroll") %>% 
  # lager overskrifter spesifisert for hver kollonne 
    add_header_row(values = c("", "Pre", "Pre", "Post", "Post")) %>%
  merge_at(part = "header", i = 1, j = 2:3) %>%
    merge_at(part = "header", i = 1, j = 4:5) %>% #justerer skrifttype
    add_footer_row(values = "Data er gitt som gjennomsnitt og standardavvik.", colwidths = 5) %>% #legger til fornote
